...
@pytest.mark.parametrize("title, content, published",[
    ("awesome new title", "awsome new content", True),
    ("awesome new title pizaa", "peperoni", True),
    ("awesome new titleadasdadasd", "adadadadas", True)
...
def test_create_post_default_published_true(authorized_client, test_user,test_posts):
    res = authorized_client.post("/posts/", json={"title": "arbitary title","content": "asdgagafhafdga"})

    created_post = schemas.Post(**res.json())
    assert res.status_code == 201
    assert created_post.title == "arbitary title"
    assert created_post.content == "asdgagafhafdga"
    assert created_post.published == True
    assert created_post. owner_id == test_user['id']


def test_unauthorized_user_create_posts(client, test_user, test_posts):
    res = client.post("/posts/", json={"title": "arbitary title","content": "asdgagafhafdga"})
    assert res.status_code == 401
